...
class IntConst(Expr):
    ...
    def __init__(self, integer: int) -> int:
        self.integer = integer

    def  eval(self) -> 'IntConst':
        return IntConst(self.integer)

    def __str__(self):
        return f'{self.integer}'

    def __repr__(self):
        return f'IntConst({self.integer})'

    def __eq__(self, other: Expr):
        return isinstance(other, IntConst) and self.integer == other.integer
...
class Plus(Expr):
    ...
    def __repr__(self):
        return f'Plus(IntConst({self.left}), IntConst({self.right}))'
    ...
    def eval(self) -> "IntConst":
        ...
        return IntConst(left_val.integer + right_val.integer)
...
class BinOp(Expr):
    ...
    def eval(self) -> "IntConst":
        ...
        left_val = self.left.eval()
        right_val = self.right.eval()
        return IntConst(self._apply(left_val.integer, right_val.integer))

    def __repr__(self) -> str:
        return f'{(self.op_name)}(IntConst({(self.left)}), IntConst({(self.right)}))'
...
class Unop:
    ...
    def eval(self) -> "IntConst":
        """Each concrete subclass must define _apply(int, int)->int"""
        left_val = self.left.eval()
        return IntConst(self._apply(left_val.integer))

    def __repr__(self) -> str:
        return f'{self.op_name}(IntConst({self.left}))'
